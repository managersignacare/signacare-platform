...
function writeApiSrc(relPath: string, content: string): void {
  const abs = join(workdir, 'apps/api/src', relPath);
  mkdirSync(join(abs, '..'), { recursive: true });
  writeFileSync(abs, content);
}
...
`,
  ...
  it('DR-7b: generated db type file is inferred from apps/api/src/db/types/<table>.ts', () => {
    const snap = writeSnapshot({ patient_invites: ['id', 'clinic_id', 'code'] });
    writeApiSrc(
      'db/types/patient_invites.ts',
...
`,
  ...
  it('DR-7c: stale generated db type fails reverse-drift instead of becoming unbound', () => {
    const snap = writeSnapshot({ patient_invites: ['id', 'clinic_id', 'code', 'expires_at'] });
    writeApiSrc(
      'db/types/patient_invites.ts',
      `// AUTO-GENERATED by scripts/generate-types-from-migrations.ts
export interface PatientInvitesRow {
