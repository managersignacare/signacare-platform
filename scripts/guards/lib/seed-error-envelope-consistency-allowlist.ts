...
const ALLOWLIST_PATH = path.join(__dirname, '..', 'check-error-envelope-consistency.allowlist');

function main(): void {
  const result = runGuard();
  if (result.exitCode === 2) { console.error('✗ exit 2'); process.exit(2); }
  const entries: { file: string; fingerprint: string; status: number; line: number }[] = [];
  for (const v of result.violations) {
    ...
    if (!fp) continue;
    entries.push({ file: v.file, fingerprint: fp, status: v.status, line: v.lineNo });
  }
  entries.sort((a, b) => (a.file === b.file ? a.line - b.line : a.file.localeCompare(b.file)));
  const lines = entries.map(
    (e) => `${e.file} ${e.fingerprint}  # PR-R1-21 baseline original-lineno:${e.line} status=${e.status} | BUG-PR-R1-21-CASCADE-DRAIN-ERROR-ENVELOPE — pre-existing inline error response; refactor to next(new AppError(...)) per CLAUDE.md §3.4`,
  );
