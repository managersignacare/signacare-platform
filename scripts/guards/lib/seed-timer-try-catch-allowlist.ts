...
const ALLOWLIST_PATH = path.join(__dirname, '..', 'check-timer-try-catch.allowlist');
...
function main(): void {
  ...
  const lines = entries.map(
    (e) => `${e.file} ${e.fingerprint}  # PR-R1-24 baseline original-lineno:${e.line} | BUG-PR-R1-24-CASCADE-DRAIN-TIMER-TRY-CATCH — pre-existing async timer without try/catch wrap; per CLAUDE.md §3.2`,
  );
