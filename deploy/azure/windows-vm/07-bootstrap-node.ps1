# -----------------------------------------------------------------------------
# Signacare EMR - One-shot Windows node bootstrap
# 07-bootstrap-node.ps1
#
# Purpose:
#   Deterministically bootstrap one VM node from a packaged release bundle,
#   wiring runtime secrets through Azure Key Vault, then validating local boot.
#
# Flow:
#   1) Run prerequisites (01)
#   2) Stage app bundle on D: drive
#   3) Create DB + roles with deterministic passwords (02)
#   4) Write production .env with SECRETS_BACKEND=azure_keyvault
#   5) Install app + migrations (03)
#   6) Configure IIS reverse proxy (04)
#   7) Install/start Windows service (05)
#   8) Relocate Redis persistence (06)
...
param(
    ...
    [Parameter()] [string]$DbOwnerPassword,
    [Parameter()] [string]$DbAppPassword,
...
function Resolve-DbPasswordsFromKeyVault {
    ...
    $kvBase = $KeyVaultUrl.TrimEnd('/')
    $owner = (Invoke-RestMethod -Method GET -Headers $kvHeaders -Uri "$kvBase/secrets/db-password?api-version=7.4").value
    $app = (Invoke-RestMethod -Method GET -Headers $kvHeaders -Uri "$kvBase/secrets/db-app-password?api-version=7.4").value
    if ([string]::IsNullOrWhiteSpace($owner) -or [string]::IsNullOrWhiteSpace($app)) {
...
function Resolve-DbSuperuserPassword {
    ...
    try {
        ...
        $kvHeaders = @{ Authorization = "Bearer $($tokenResp.access_token)" }
        $kvBase = $KeyVaultUrl.TrimEnd('/')
        $kvSuper = (Invoke-RestMethod -Method GET -Headers $kvHeaders -Uri "$kvBase/secrets/db-superuser-password?api-version=7.4").value
        if (-not [string]::IsNullOrWhiteSpace($kvSuper)) {
            Write-Host "  Using postgres superuser secret from Key Vault: db-superuser-password"
            return $kvSuper.Trim()
        }
        Write-Host '  db-superuser-password not configured in Key Vault.'
    } catch {
        Write-Host "  Key Vault lookup for db-superuser-password failed: $($_.Exception.Message)"
    }
...
function Get-PostgresServiceStatus {
    ...
    $probe = 'C:\Setup\bootstrap-db-probe.sql'
    Set-Content -Path $probe -Value "SELECT 1;" -Encoding UTF8
    $stdout = 'C:\Setup\bootstrap-db-probe-stdout.log'
    $stderr = 'C:\Setup\bootstrap-db-probe-stderr.log'
...
$logPath = Join-Path $setupRoot 'bootstrap-node.log'
...
try {
    Write-Step '1/9 - Download + extract release bundle'
    ...
    $scriptRoot = Join-Path $bundleDir.FullName 'deploy\azure\windows-vm'

    Write-Step '2/9 - Run prerequisites script (01)'
    $script01 = Join-Path $scriptRoot '01-setup-prerequisites.ps1'
    Assert-PathExists $script01 "Missing script: $script01"
    & $script01 *>> $logPath

    Write-Step '3/9 - Stage application bundle to D:\Signacare\app'
    ...
    Write-Step '4/9 - Create/repair database roles and schema baseline (02)'
    if ([string]::IsNullOrWhiteSpace($DbOwnerPassword) -or [string]::IsNullOrWhiteSpace($DbAppPassword)) {
        Write-Step 'Resolving DB role passwords from Key Vault via managed identity'
        $resolvedPw = Resolve-DbPasswordsFromKeyVault -KeyVaultUrl $AzureKeyVaultUrl
        if ([string]::IsNullOrWhiteSpace($DbOwnerPassword)) { $DbOwnerPassword = $resolvedPw.Owner }
        if ([string]::IsNullOrWhiteSpace($DbAppPassword)) { $DbAppPassword = $resolvedPw.App }
    }
    ...
    if ([string]::IsNullOrWhiteSpace($superPw)) {
        Write-Warning 'Postgres superuser password not found (local file or Key Vault secret: db-superuser-password).'
        ...
        if (Get-PostgresServiceStatus -OwnerPassword $DbOwnerPassword) {
            ...
            throw 'Missing postgres superuser secret and DB owner probe failed. Store db-superuser-password in Key Vault OR re-run with a fresh Postgres bootstrap context.'
        }
    } else {
        & $script02 `
    ...
    Write-Step '5/9 - Write production API env (Key Vault backend)'
...
"@
    ...
    Write-Step '6/9 - Install app dependencies + run migrations (03)'
    ...
    Write-Step '7/9 - Configure IIS reverse proxy (04)'
    ...
    Write-Step '8/9 - Install/start Windows API service + Redis data-dir config (05,06)'
    ...
    Write-Step '9/9 - Local boot validation'
