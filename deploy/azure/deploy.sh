...
#      storage-secret-key into the Key Vault for the BlobStorage S3
#      backend.
...
az deployment sub create \
  ...
  --parameters @"$PARAMS" \
  --parameters adminPasswordSecret="$ADMIN_PASSWORD_SECRET" \
  >/tmp/signacare-deploy-"$DEPLOY_NAME".json
DEPLOY_EXIT=$?
set -e
...
# Storage account key for the BlobStorage S3-compatible backend.
...
echo " Next steps:"
echo "──────────────────────────────────────────────────────────────────────"
echo " 1. Push container images to the ACR ${NAME_PREFIX}cr${ENV}.azurecr.io:"
echo "      docker buildx build --platform linux/amd64 -f apps/api/Dockerfile \\"
echo "        -t ${NAME_PREFIX}cr${ENV}.azurecr.io/signacare-api:latest \\"
echo "        -t ${NAME_PREFIX}cr${ENV}.azurecr.io/signacare-api:$(git rev-parse --short HEAD) \\"
echo "        --push ."
echo "      docker buildx build --platform linux/amd64 -f apps/web/Dockerfile --build-arg VITE_API_URL=\"https://${NAME_PREFIX}-api-${ENV}.azurewebsites.net/api\" \\"
echo "        -t ${NAME_PREFIX}cr${ENV}.azurecr.io/signacare-web:latest \\"
echo "        -t ${NAME_PREFIX}cr${ENV}.azurecr.io/signacare-web:$(git rev-parse --short HEAD) \\"
echo "        --push ."
echo "      az acr login --name ${NAME_PREFIX}cr${ENV}"
  echo " 2. Point the API and Web sites at the new images:"
  echo "      az webapp config container set --name ${NAME_PREFIX}-api-${ENV} \\"
  echo "        --resource-group $RG_NAME \\"
  echo "        --docker-custom-image-name ${NAME_PREFIX}cr${ENV}.azurecr.io/signacare-api:latest"
  echo "      az webapp config container set --name ${NAME_PREFIX}-web-${ENV} \\"
  echo "        --resource-group $RG_NAME \\"
  echo "        --docker-custom-image-name ${NAME_PREFIX}cr${ENV}.azurecr.io/signacare-web:latest"
echo " 3. Run the smoke test:"
echo "      ENV=$ENV deploy/azure/post-deploy-smoke.sh"
echo " 4. Replace placeholder secrets (sentry-dsn, slack-webhook-*) in the Key Vault."
echo " 5. Rotate the ADMIN_PASSWORD_SECRET out of your shell history:"
