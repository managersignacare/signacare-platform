// AUTO-GENERATED scaffold by scripts/generate-types-from-migrations.ts
// DO NOT EDIT MANUALLY. Hand-written Response must `extends` this
// scaffold's Zod schema OR carry a `// @scaffold-divergence: <reason>`
// annotation. The scaffold-extension guard (Phase 0b.1b) enforces this.

import { z } from 'zod';

export const ReportSchedulesResponseScaffoldSchema = z.object({
  id: z.string().uuid(),
  clinicId: z.string().uuid(),
  reportType: z.string().max(100),
  name: z.string().max(200),
  frequency: z.string().max(30),
  scheduleCron: z.string().max(100).nullable().optional(),
  recipients: z.unknown(),
  filters: z.unknown(),
  format: z.string().max(20),
  isActive: z.boolean(),
  createdByStaffId: z.string().uuid().nullable().optional(),
  lastRunAt: z.string().datetime().nullable().optional(),
  nextRunAt: z.string().datetime().nullable().optional(),
  createdAt: z.string().datetime(),
  updatedAt: z.string().datetime(),
});

export type ReportSchedulesResponseScaffold = z.infer<typeof ReportSchedulesResponseScaffoldSchema>;
